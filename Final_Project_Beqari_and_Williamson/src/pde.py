...
class ODENetwork(tf.keras.Model):
    ...
    def loss_ic(self, point):
        ...
        point_x1 = tf.math.multiply(point, tf.constant(np.array([1, 0]), dtype=tf.double))
        point_x1 = tf.math.add(point_x1, tf.constant(np.array([0, 1]), dtype=tf.double))
        point_x1 = tf.expand_dims(point_x1, axis=0)

        point_0t = tf.math.multiply(point, tf.constant(np.array([0, 1]), dtype=tf.double))
        point_0t = tf.expand_dims(point_0t, axis=0)

        point_1t = tf.math.multiply(point, tf.constant(np.array([0, 1]), dtype=tf.double))
    ...
    def train_step(self, points):
        ...
        with tf.GradientTape(persistent=True) as tape_ord_2:
            ...
            with tf.GradientTape(persistent=True) as tape_ord_1:
                ...
                loss_ic = tf.keras.backend.map_fn(
                    lambda x: self.loss_ic(x), (points), dtype=tf.double
                )
